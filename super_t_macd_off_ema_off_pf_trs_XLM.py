...
def check_pos(pair_num):
    ...
    if profit_factor == 2 and pos_amt != 0:
        ...
        price = ohlcv[-1][4]
        un_pr = float(pos["unrealizedProfit"])
        if pos_amt > 0 and df['St_d2'][candle_limit-1] == 1 \
                or un_pr >= loss_amt * profit_factor:
...
def strategy():
    global buy_flag, sell_flag, ema_angle, size, df, df_i, entry_price, stop_price, take_price, profit_factor, loss_amt
...
def open_buy_market_order(pair, size, stop_price, take_price):
    ...
    try:
        ...
        if profit_factor == 1:
            ...
            print(f'Успешно установлен тейк-профит {round(take_price, 6)} ')
            #print(take_order)
