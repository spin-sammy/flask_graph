...
def read_pair_data(pair):  # ЧИТАЕМ ДАННЫЕ ПАРЫ
    ...
    df.index = np.arange(len(df)) # Перенумеровали номера свечей
...
while True:
    ...
    if current_time[4] != old_time[4]:  # Проверяем паттерны каждую минуту
        ...
        if not sell_flag and not buy_flag:
            strategy()
        print(f'Позиция: {pos["positionAmt"]} угол EMA: {round(ema_angle, 2)} лимит: {ema_angle_limit} Sell: {sell_flag} Buy: {buy_flag}')
        print(f'EMA: {round(df["Ema"][candle_limit-1], 2)} MACD: {round(df["Macd"][candle_limit-1], 2)} MACD_s: {round(df["Macd_s"][candle_limit-1],2)}')
